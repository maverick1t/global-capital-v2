...
export const fetchGlobalNews = async (
  ...
  // OPTIMIZED PROMPT FOR ELITE PERSONA
  const prompt = `
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        tools: [{ googleSearch: {} }],
        temperature: 0.3,
        // REMOVED responseMimeType: "application/json" because it conflicts with Google Search tool in some regions/models
      },
    });
    ...
    const items: NewsItem[] = Array.isArray(rawItems) ? rawItems.map((item: any, index: number) => ({
      id: `${category}-${Date.now()}-${index}`, // Unique ID per category
